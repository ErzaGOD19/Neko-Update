...
fn check_updates() -> io::Result<()> {
    ...
    let _ = run_command("xbps-install", &["-S"], false);

    let mut total_count = 0;
    if let Ok((_, output)) = run_command("xbps-install", &["-un"], true) {
    ...
    if Path::new("/usr/bin/flatpak").exists() {
        if let Ok((_, output)) = run_command("flatpak", &["update", "--noninteractive"], true) {
             if !output.contains("Nothing to do") {
                 let flatpak_count = output.lines().filter(|l| l.contains(".") && (l.contains("/") || l.contains("update"))).count();
                 log(&format!("Flatpak encontró {} actualizaciones", flatpak_count));
                 total_count += flatpak_count;
             }
        }
    }
...
fn parse_update_count(output: &str) -> usize {
    let count = output.lines().filter(|line| line.contains(" -> ")).count();

    if count == 0 {
        for line in output.lines() {
            let line = line.trim().to_lowercase();
            if line.contains("nothing to do") || line.contains("no packages") {
                return 0;
            }
        }
    }
...
fn clean_system() -> io::Result<()> {
    ...
    let _ = run_command("xbps-remove", &["-Oo", "-y"], false);
...
fn apply_updates() -> io::Result<()> {
    ...
    let _ = run_command("xbps-remove", &["-Oo"], false);
